...
public class FileUtils {
    ...
    public static void copyFile(String srcFileName, String descFileName) {
        FileUtils.copyFileCover(srcFileName, descFileName, false);
    }
     ...
     * @return 如果复制成功，则返回true，否则返回false
     */
    public static void copyFileCover(String srcFileName, String descFileName, boolean coverlay) {
    ...
    public static boolean copyDirectoryCover(String srcDirName,
        ...
        for (int i = 0; i < files.length; i++) {
            // 如果是一个单个文件，则直接复制
            if (files[i].isFile()) {
                FileUtils.copyFile(files[i].getAbsolutePath(),
    ...
    public static boolean deleteDirectory(String dirName) {
        ...
        // 删除当前目录
        if (dirFile.delete()) {
            log.error("删除目录 " + dirName + " 成功!");
            return true;
        } else {
            log.error("删除目录 " + dirName + " 失败!");
            return false;
        }
     ...
     * @return
     */
    public static List<TreeNode> getFileList(String strPath, List<TreeNode> treeList, TreeNode pNode) {
        ...
        if (files != null) {
            for (int i = 0; i < files.length; i++) {
                if (files[i].isDirectory()) { // 判断是文件还是文件夹
                    ...
                    node.setParentId(pNode.getId());//父节点的Id
                    node.setId(files[i].getAbsolutePath());
                    node.setName(files[i].getName());
                    treeList.add(node);
                    getFileList(files[i].getAbsolutePath(), treeList, node); // 获取文件绝对路径
                }
            }
        }
        ...
        try {
            ...
            if (file.isFile()) {// 表示该文件不是文件夹
                ...
                for (String childFilePath : childFilePaths) {
                    String childPath = file.getAbsolutePath() + "/" + childFilePath;
