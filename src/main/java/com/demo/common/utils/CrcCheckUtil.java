...
/**
 * CRC校验，校验参数是否计算值与传输值是否一致，简单对参数的合法性进行校验，防止篡改参数
 * <p>
 * <p>
 * Created by  jky on 2021-04-26.
 */
public class CrcCheckUtil {

    private static int CRC16table[] = {0x0000, 0xC0C1, 0xC181, 0x0140, 0xC301,
     ...
     * @return
     */
    public static int CRC16_isr(byte[] dat) {
        int TxCRC16 = 0;
        for (int i = 0; i < dat.length; i++) {
            ...
            TxCRC16 = (CRC16table[num ^ (TxCRC16 & 0xFF)] ^ (TxCRC16 / 0x100));
        }
        if (TxCRC16 == 0x0000) {
            TxCRC16 = 0xFFFF;
        }
    ...
    public static int checkNum(String... src) {
        ...
        for (int i = 0; i < src.length; i++) {
            String tempParam = src[i];
            if (!StringUtil.isEmpty(tempParam)) {
                int temp = CRC16_isr(tempParam.getBytes());
                num = num ^ temp;
            }
        }
        return num;
